...
DEV_I2C_Port DEV_I2C_Init()
{
    ...
    ESP_LOGI(TAG, "I2C bus initialized on SDA=%d, SCL=%d", EXAMPLE_I2C_MASTER_SDA, EXAMPLE_I2C_MASTER_SCL);
...
void DEV_I2C_Write_Nbyte(i2c_master_dev_handle_t dev_handle, uint8_t *pdata, uint8_t len)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C write failed: %s (handle=%p)", esp_err_to_name(ret), dev_handle);
    }
}
...
void DEV_I2C_Read_Nbyte(i2c_master_dev_handle_t dev_handle, uint8_t Cmd, uint8_t *pdata, uint8_t len)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C read Nbyte failed: %s", esp_err_to_name(ret));
    }
}
