...
- **`GET /api/power-control`** — Current power control state (mode, watts).
- **`POST /api/power-control`** — Set mode. Body: `{"mode": "import"|"export"|"off", "watts": 0-11400}`. Persists; background refreshes Solis every 2 min (dead-man). Use for reliable grid charge or export.
