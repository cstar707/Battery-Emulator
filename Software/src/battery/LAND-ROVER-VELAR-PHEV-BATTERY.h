...
#define VELAR_CONTACTOR_DELAY_MS 5000
...
class LandRoverVelarPhevBattery : public CanBattery {
 ...
 private:
  ...
  // PCM_PMZ_HVBatt (0xA2, 50ms). Per VELAR_PMZ_HSCAN.dbc:
  ...
  // BCCM_PMZ_DCDCOperatingMode (0x96, 10ms). ChargerHVILStatus bits 33-34 = 0 (OK).
  // BCCM normally sends; emulator sends when no charger present.
  ...
  // Inverter_PMZ_InverterHV (0xA4, 20ms). InverterHVILStatus at start bit 37, 2 bits.
  // Value 2 = OK/Closed. Byte 4 = 0x20 → InverterHVILStatus=2.
