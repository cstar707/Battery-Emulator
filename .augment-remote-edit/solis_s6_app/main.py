...
def _on_solark_mqtt_message(_client, _userdata, msg):
    ...
    Handles two formats:
      1. JSON blob on  solar/solark          (Battery-Emulator board, .15)
      2. Individual    solar/solark/sensors/<name>  (ESPHome at .48)
    """
    ...
    try:
        ...
        if "battery_soc_pptt" in _solark_sensor_scratch:
            _solark_cache["data"] = dict(_solark_sensor_scratch)
            _solark_cache["ts"] = time.time()
            _solark_cache["ok"] = True
...
def _run_solark_mqtt_subscriber() -> None:
    ...
    try:
        ...
        # Subscribe to JSON blob (BE board) and individual sensor topics (ESPHome)
        topics = []
...
def _poll_sync() -> None:
    ...
    try:
        ...
        # Run automations using whatever solark data we have
